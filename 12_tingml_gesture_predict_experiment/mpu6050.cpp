...
#endif
uint8_t dev = 0;
uint8_t mpu_addr = MPU6050_DEFAULT_ADDRESS;
uint8_t buffer[14];

int8_t readBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *data)
{
    return i2c_read_regs(dev, devAddr, regAddr, data, length, 0);
}
...
bool writeBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t* data) {
    return i2c_write_regs(dev, devAddr, regAddr, data, length, 0);
}
...
uint8_t writeBits(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t data) {
    ...
    uint8_t b;
    if (readByte(devAddr, regAddr, &b) == I2C_ACK) {
...
int8_t readBits(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t *data) 
{
    ...
    if ((count = readByte(devAddr, regAddr, &b)) == I2C_ACK) {
        uint8_t mask = ((1 << length) - 1) << (bitStart - length + 1);
        LOG_DEBUG("[mpu6050]B:0x%x, mask:0x0%x\n", b, mask);
        b &= mask;
        b >>= (bitStart - length + 1);
        *data = b;
    }
...
void setClockSource(uint8_t source) {
    writeBits(mpu_addr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, source);
}


void setFullScaleGyroRange(uint8_t range) {
    writeBits(mpu_addr, MPU6050_RA_GYRO_CONFIG, MPU6050_GCONFIG_FS_SEL_BIT, MPU6050_GCONFIG_FS_SEL_LENGTH, range);
}
void setFullScaleAccelRange(uint8_t range) {
    writeBits(mpu_addr, MPU6050_RA_ACCEL_CONFIG, MPU6050_ACONFIG_AFS_SEL_BIT, MPU6050_ACONFIG_AFS_SEL_LENGTH, range);
}
void setSleepEnabled(bool enabled) {
    writeBit(mpu_addr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_SLEEP_BIT, enabled);
}
uint8_t getDeviceID(void) {
    readBits(mpu_addr, MPU6050_RA_WHO_AM_I, MPU6050_WHO_AM_I_BIT, MPU6050_WHO_AM_I_LENGTH, buffer);
    return buffer[0];
}
 ...
 */
MPU6050::MPU6050(uint8_t dev, uint8_t address) {
    i2c_dev = dev;
    devAddr = address;
}
...
void MPU6050::reset(void) {
    ...
    xtimer_msleep(100);
    writeBit(devAddr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_DEVICE_RESET_BIT, false);
}
...
uint16_t MPU6050::getRate() {
    readByte(devAddr, MPU6050_RA_SMPLRT_DIV, buffer);
    uint16_t sample_rate = 1000 / (uint16_t)(1 + buffer[0]); 
    return sample_rate;
}

void MPU6050::setRate(uint16_t rate) {
    uint8_t data;
    if (rate > 1000) 
        rate = 1000;
    else if (rate < 4)
        rate = 4;
    data = 1000 / rate - 1;
    writeByte(devAddr, MPU6050_RA_SMPLRT_DIV, data);
}
...
void MPU6050::getMotion6(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz) {
    ...
    *ax = (((int16_t)buffer[0]) << 8) | (int16_t)buffer[1];
    *ay = (((int16_t)buffer[2]) << 8) | (int16_t)buffer[3];
    *az = (((int16_t)buffer[4]) << 8) | (int16_t)buffer[5];

    *gx = (((int16_t)buffer[8]) << 8) | (int16_t)buffer[9];
    *gy = (((int16_t)buffer[10]) << 8) | (int16_t)buffer[11];
    *gz = (((int16_t)buffer[12]) << 8) | (int16_t)buffer[13];
}
...
float MPU6050::getTemperature() {
    ...
    uint16_t regval = (((int16_t)buffer[0]) << 8) | buffer[1];
    float temperature = 36.53 + regval / 340.0;
    return temperature;
}
